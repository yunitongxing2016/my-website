const cityGroups = {
  ...
  "陕西": ["西安", "铜川", "宝鸡", "咸阳", "渭南", "汉中", "延安", "榆林", "安康", "商洛"],
...
const weatherTexts = ["晴", "多云", "阴", "小雨", "阵雨"];
const studyTips = [
  "适合读书和写学习笔记",
  "适合整理作品集",
  "适合在家练 Python",
...
function getDemoCities() {
  return Object.values(cityGroups).flat();
}

function cityCode(city) {
  return Array.from(city).reduce((total, char) => total + char.charCodeAt(0), 0);
}

function getDemoWeather(city) {
  const fallbackCity = "上海";
  const selectedCity = getDemoCities().includes(city) ? city : fallbackCity;
  const code = cityCode(selectedCity);

  return {
    city: selectedCity,
    temperature: `${18 + (code % 16)}°C`,
    windSpeed: `${6 + (code % 16)} km/h`,
    weatherText: weatherTexts[code % weatherTexts.length],
    tip: studyTips[code % studyTips.length]
  };
}
